...
import { cdpConfigFor, type Product } from "@/lib/cdp-config";
 ...
 *
 * `product` picks the CDP project and the session endpoint. Basemate and iStonk
 * are different CDP projects, so the same email is a different end user (and
 * smart account) in each — signing an iStonks page in on the Basemate project
 * would link the Basemate wallet.
 */
...
const PRODUCT_COPY: Record<
  Product,
  { name: string; sessionPath: string; emailHint: string; notLinked: string }
> = {
  basemate: {
    name: "Basemate",
    sessionPath: "/api/app/session",
    emailHint: "Use the email you set up with Basemate in iMessage.",
    notLinked:
      "No Basemate account is linked to this email yet. Text Basemate in iMessage to set up first, then come back.",
  },
  istonk: {
    name: "iStonk",
    sessionPath: "/api/istonks/session",
    emailHint: "Use the email you set up with iStonk in iMessage.",
    notLinked:
      "No iStonk wallet is linked to this email yet. Text iStonk in iMessage and reply connect first, then come back.",
  },
};
...
export function SignInDialog({
  ...
  if (!open) return null;
  return (
    <CDPReactProvider config={cdpConfigFor(product)}>
      <SignInDialogInner product={product} onClose={onClose} onSuccess={onSuccess} />
    </CDPReactProvider>
  );
}
...
function SignInDialogInner({
  ...
  const linkSession = useCallback(async () => {
    ...
    try {
      ...
      if (!accessToken) throw new Error(`Couldn't read your ${copy.name} session.`);

      const res = await fetch(copy.sessionPath, {
        method: "POST",
      ...
      setMessage(
        /no basemate account is linked|not linked|connect it from basemate|no wallet found/i.test(raw)
          ? copy.notLinked
          : raw,
      );
    }
  }, [copy, currentUser, getAccessToken, onSuccess]);
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Sign in to ${copy.name}`}
      >
